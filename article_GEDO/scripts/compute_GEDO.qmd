...
saveRDS(meta_data, paste0(folder_for_res,"/meta_data.rds"))
...
# 1) Préparation
  ...
  message(sprintf("Proportion de cellules modifiées : %.1f%%", pct_modif))
  
  cat("printing heatmap modification")
    ...
    plot_noized_data=plot_noized_data +
      ...
      theme_cadre
    # ggsave(plot = plot_noized_data, filename = paste0(folder_for_res, "heatmap_noized_data.pdf"))
    
  saveRDS(object = as_grob(plot_noized_data), file=paste0(folder_for_res,"g.rds"))

  
  # 5) Standardisation 0-1
  ...
  # 6) Vérification
  cat(sprintf("Biais appliqué sur %d gènes × %d individus\n",
...
matrix_list = list(GEDO=gedo_obj,
                   ...
                   MZS=mean_z_score_module_matrix,
                   MZS_noized=noized_mean_z_score_module_matrix,
                   ssGSEA = ssgsea_obj,
...
res$dt_auc_results[,metric:=factor(metric, levels=names(matrix_list_without_noize))]
...
best            <- levels_metric[2]
others          <- levels_metric[-2]
...
plot_boxplot_auc <- ggplot(res$dt_auc_results,
                           aes(x = metric, y = auc, color = metric)) +
...
# print(res$plot)
# plot_module_auc_comp=res$plot

# plot_boxplot_auc = res$boxplot
# plot_auc_module_comp_reg = res$combined_subplots


# if(file.exists(file=paste0(folder_for_res, "module_auc_comparison_with_noize.rds"))){
#   res_noize=readRDS(file=paste0(folder_for_res, "module_auc_comparison_with_noize.rds"))
# }else{
# res_noize=compute_auc_modules(matrix_list = matrix_list_without_noize)
# saveRDS(res_noize, file=paste0(folder_for_res, "module_auc_comparison_with_noize.rds"))
# }
...
saveRDS(object = as_grob(plot_boxplot_auc_noized), file=paste0(folder_for_res,"h.rds"))


# # enregistrement
# ggsave(
#   filename = file.path(folder_for_res, "module_auc_comparison_with_noize.pdf"),
#   plot     = plot,
#   height   = 7,
#   width    = 15
# )
...
plot_auc_rf_knn_noize <- ggplot(long, aes(x = base, y = AUC, fill = metric)) +
  ...
  scale_fill_manual(values = c("Without bias"="#000004FF","With bias"="#BB3754FF","Difference"="#FDE725FF"))+
  
  geom_text(
  data        = stars_df,
  aes(x = base, y = 1-0.01, label = Signif),  
  inherit.aes = FALSE,
  vjust       = -0.5,
  hjust       = 0.5
) +
...
saveRDS(object = h_grob, file=paste0(folder_for_res,"i.rds"))


#   ggsave(plot,file=paste0(folder_for_res,"auc_noized_comp_rf_knn.pdf"), width = 10, height = 15)
# print(plot)
...
meta_data = readRDS(paste0(folder_for_res, "/meta_data.rds"))

#-------------Preparing module matrix UMAP

if(!file.exists(paste0(folder_for_res, "matrix_list_umap.rds"))){
matrix_list_umap<- lapply(matrix_list_without_noize, function(x) {
  set.seed(123)
  umap = data.table(uwot::umap(X = x$module_matrix,n_components = 10,n_neighbors = 10, metric = "euclidean"))
  x$module_matrix = umap
  return(x)
})
saveRDS(matrix_list_umap, file = paste0(folder_for_res, "matrix_list_umap.rds"))
}else{
  matrix_list_umap=readRDS(paste0(folder_for_res, "matrix_list_umap.rds"))
}


if(!file.exists(paste0(folder_for_res, "matrix_list_sjd_umap.rds"))){
  matrix_list_sjd_umap <- lapply(matrix_list_without_noize, function(x) {
    idx <- x$diag == "SjD"
  ...
  matrix_list_sjd_umap=readRDS(paste0(folder_for_res, "matrix_list_sjd_umap.rds"))
}
...
if(!file.exists(paste0(folder_for_res, "knn_ifn.rds"))){
  knn_ifn = compute_regression_with_cv(model = "knn", k=k_ctrl_sjd, k_folds = 50, dt_list=matrix_list_umap, num_cores=num_cores, y=meta_data$IFN_score)
...
if(!file.exists(paste0(folder_for_res, "knn_ifn_sjd.rds"))){
  stopifnot(
    nrow(matrix_list_sjd_umap[[1]]$module_matrix) ==
      sum(meta_data$diag == "SjD")
  )
  
  knn_ifn_sjd = compute_regression_with_cv(model = "knn", k=k_sjd, k_folds = 50, dt_list=matrix_list_sjd_umap, num_cores=num_cores, y= y_ifn_sjd)
...
saveRDS(object = as_grob(plot_knn_ifn), file=paste0(folder_for_res,"e.rds"))
...
cat("e")
  ph_e = heatmap.ifn(gedo_obj = matrix_list$MZS,
                     IFN_score=ifn_score,
                     title="",
                     legend=T,
                     annotation_legend =T)
...
if(file.exists(file.path(folder_for_res, "combined_heatmaps.png"))==F){
...
gt_g <- readRDS(paste0(folder_for_res, "gt_g.rds"))
  ...
  # a = wrap_elements(full = gt_a)
  # b = wrap_elements(full = gt_b)
  # c = wrap_elements(full = gt_c)
  # d = wrap_elements(full = gt_d)
  # e = wrap_elements(full = gt_e)
  # f = wrap_elements(full = gt_f)
  # g = wrap_elements(full = gt_g)
...
ggsave(
  filename = file.path(folder_for_res, "combined_heatmaps.jpg"),
  plot     = comb,
  width    = 20,
  height   = 20,
  units    = "in",
...
ggsave(
  filename = file.path(folder_for_res, "combined_heatmaps.pdf"),
  plot     = comb,
  width    = 20,
  height   = 20,
  units    = "in",
...
library(DBCVindex)
cl_res = compute_clustering_quality(matrix_list = matrix_list_without_noize, num_cores=num_cores)
...
saveRDS(object = as_grob(clustering_plot), file=paste0(folder_for_res,"f.rds"))
...
i_g <- readRDS(file.path(folder_for_res, "i.rds"))
...
i_w <- wrap_elements(full = i_g)


abcd  <- a_w + b_w  + c_w + d_w              + plot_layout(ncol = 4)
ef <- e_w + f_w           + plot_layout(ncol = 2)
ghi <- g_w + h_w + i_w          + plot_layout(ncol = 3, widths = c(0.2, 0.4,0.4))
...
ggsave(
  filename = file.path(folder_for_res, "combined_figure_results.png"),
  plot     = comb,
  width    = 35,   
  height   = 20
)
...
# 14. Enrichment of clinical features in PHATE visualizations
...
plot_gedo= plot_phate(data=matrix_list_without_noize$GEDO$module_matrix, k = 15, meta_data = PS_brutes, omic_id=omic_id)
...
#| fig.align: "center"
# use_python("/shared/software/miniconda/envs/r-4.4.1/bin/python", required = TRUE)
plot_gedo_corr= plot_phate(data=matrix_list_without_noize$GEDOcorr$module_matrix, k = 15, meta_data = PS_brutes,  omic_id=omic_id)
...
plot_umap_gedo= plot_phate(data=matrix_list_without_noize$UMAP_GEDO$module_matrix, k = 15, meta_data = PS_brutes,  omic_id=omic_id)
...
plot_pca1= plot_phate(data=matrix_list_without_noize$PCA1$module_matrix, k = 15, meta_data = PS_brutes,  omic_id=omic_id)
...
#| fig.align: "center"
# use_python("/shared/software/miniconda/envs/r-4.4.1/bin/python", required = TRUE)
plot_mean_zscore= plot_phate(data=matrix_list_without_noize$MZS$module_matrix, k = 15, meta_data = PS_brutes,  omic_id=omic_id)
...
#| fig.align: "center"
# use_python("/shared/software/miniconda/envs/r-4.4.1/bin/python", required = TRUE)
plot_ssgsea= plot_phate(data=matrix_list_without_noize$ssGSEA$module_matrix, k = 15, meta_data = PS_brutes,  omic_id=omic_id)
# ggsave(plot_ssgsea$plot,file=paste0(folder_for_res,"phate_clinical_features_ssgsea.pdf"), width = 20, height = 10)
knitr::include_graphics(paste0(folder_for_res,"phate_clinical_features_ssgsea.pdf"))
```
...
#| fig.align: "center"
# use_python("/shared/software/miniconda/envs/r-4.4.1/bin/python", required = TRUE)
plot_gsva= plot_phate(data=matrix_list_without_noize$GSVA$module_matrix, k = 15, meta_data = PS_brutes,  omic_id=omic_id)
# ggsave(plot_gsva$plot,file=paste0(folder_for_res,"phate_clinical_features_gsva.pdf"), width = 20, height = 10)
knitr::include_graphics(paste0(folder_for_res,"phate_clinical_features_gsva.pdf"))
```

```{r combined phate figures}
...
plots <- list(
  GEDO      = plot_gedo$plot,
  GEDOcorr  = plot_gedo_corr$plot,
  UMAP_GEDO = plot_umap_gedo$plot,
  PCA1      = plot_pca1$plot,
  MZS       = plot_mean_zscore$plot,
  ssGSEA    = plot_ssgsea$plot,
  GSVA      = plot_gsva$plot
)
...
ggsave(plot=final_plot, filename = paste0(folder_for_res,"phate_combined.pdf"), width=15, height = 25)


```

# 15. Benchmark methods (time of running)
...
MZS = compute_module_matrix(method="mean_z_score",
  data=rna_seq_data,
  diag=diag,
  reference_group="Control",
  category="C7",
...
results_long$seconds = (results_long$minutes)*60
...
summary_stats <- data.table(results_long %>%
  ...
  summarise(
    ...
    sd_sec = sd(seconds),
    se_sec = sd_sec / sqrt(n())
  ))
summary_stats$expression = factor(names(matrix_list_without_noize), levels = names(matrix_list_without_noize))


plot = ggplot(summary_stats, aes(x = expression, y = median_sec)) +
  geom_bar(stat = "identity", fill = "white", color = "black", width = 0.6) +
  geom_errorbar(aes(ymin = median_sec - se_sec, ymax = median_sec + se_sec),width = 0.2, color = "black") +
  scale_y_break(c(3*60, 50*60), expand = FALSE, space = 1) +
  labs(x = "Methods",
       y = "Time (seconds)") +
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))
...
# ggsave(
#   filename = paste0(folder_for_res, "benchmark_res.pdf"),
#   plot     = plot,
...
pdf(
  file   = paste0(folder_for_res, "benchmark_res.pdf"),
  width  = 8,
  height = 6,
  onefile = TRUE
)
...
mean_gedo_corr = mean(summary_stats[expression =="GEDO"]$median_sec, summary_stats[expression =="GEDOcorr"]$median_sec)

#PCA1 : 

print(paste("Ratio with PCA1 : ", mean_gedo_corr / summary_stats[expression =="PCA1"]$median_sec))

print(paste("Ratio with MZS : ", mean_gedo_corr / summary_stats[expression =="MZS"]$median_sec))

print(paste("Ratio with ssGSEA : ", mean_gedo_corr / summary_stats[expression =="ssGSEA"]$median_sec))

print(paste("Ratio with GSVA : ", mean_gedo_corr / summary_stats[expression =="GSVA"]$median_sec))



#si necessaire en bash run : pdftk /home/clem/these_clement/studies/R_Thèse_clement_LBAI/11_GEDO/Out/last_run_5/benchmark_res.pdf cat 2 output /home/clem/these_clement/studies/R_Thèse_clement_LBAI/11_GEDO/Out/last_run_5/benchmark_p2.pdf
